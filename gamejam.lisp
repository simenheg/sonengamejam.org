...
(defun html-render-download-links (download-plist)
  (with-html
    (doplist (format url download-plist)
      (htm (:a :href url (:img :src (format-image format))))
      (fmt " "))))
...
(defun html-render-entry (entry)
  (with-html
    (:div
     ...
     ;; Download links
     (:p (fmt (html-render-download-links (entry-downloads entry)))))))
